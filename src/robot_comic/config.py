...
# XTTS-v2 LAN service (#438). Protocol contract: POST /tts_stream with JSON
# body {text, speaker, language} returns chunked application/octet-stream of
# raw int16 PCM @ 24 kHz mono (no WAV header). GET /speakers returns a JSON
# list of speaker keys the server can resolve to reference WAVs. The default
# URL is the workstation that hosts llama-server, on a separate port — see
# issue #436 for the service-side scripts.
XTTS_URL_ENV = "REACHY_MINI_XTTS_URL"
XTTS_DEFAULT_SPEAKER_ENV = "REACHY_MINI_XTTS_DEFAULT_SPEAKER"
XTTS_LANGUAGE_ENV = "REACHY_MINI_XTTS_LANGUAGE"
XTTS_TIMEOUT_S_ENV = "REACHY_MINI_XTTS_TIMEOUT_S"
XTTS_DEFAULT_URL = "http://astralplane.lan:8020"
XTTS_DEFAULT_SPEAKER = "tony"
XTTS_DEFAULT_LANGUAGE = "en"
XTTS_DEFAULT_TIMEOUT_S = 30.0
...
_SUPPORTED_AUDIO_COMBINATIONS: frozenset[tuple[str, str]] = frozenset(
    {
        ...
        # Composable STT + xtts-v2 LAN TTS (#438).
        (AUDIO_INPUT_MOONSHINE, AUDIO_OUTPUT_XTTS),
        (AUDIO_INPUT_FASTER_WHISPER, AUDIO_OUTPUT_XTTS),
    }
)
...
FASTER_WHISPER_VAD_START_FRAMES_ENV = "REACHY_MINI_FASTER_WHISPER_VAD_START_FRAMES"
FASTER_WHISPER_DEFAULT_VAD_START_FRAMES = 7
...
class Config:
    ...
    FASTER_WHISPER_VAD_START_FRAMES = _env_int_clamped(
        FASTER_WHISPER_VAD_START_FRAMES_ENV,
        default=FASTER_WHISPER_DEFAULT_VAD_START_FRAMES,
        lo=1,
        hi=50,
    )
    ...
    XTTS_URL = os.getenv(XTTS_URL_ENV, XTTS_DEFAULT_URL)
    XTTS_DEFAULT_SPEAKER_KEY = os.getenv(XTTS_DEFAULT_SPEAKER_ENV, XTTS_DEFAULT_SPEAKER)
    XTTS_LANGUAGE = os.getenv(XTTS_LANGUAGE_ENV, XTTS_DEFAULT_LANGUAGE)
    XTTS_TIMEOUT_S = float(os.getenv(XTTS_TIMEOUT_S_ENV, str(XTTS_DEFAULT_TIMEOUT_S)))
...
def refresh_runtime_config_from_env() -> None:
    ...
    config.FASTER_WHISPER_VAD_START_FRAMES = _env_int_clamped(
        FASTER_WHISPER_VAD_START_FRAMES_ENV,
        default=FASTER_WHISPER_DEFAULT_VAD_START_FRAMES,
        lo=1,
        hi=50,
    ...
    config.XTTS_URL = os.getenv(XTTS_URL_ENV, XTTS_DEFAULT_URL)
    config.XTTS_DEFAULT_SPEAKER_KEY = os.getenv(XTTS_DEFAULT_SPEAKER_ENV, XTTS_DEFAULT_SPEAKER)
    config.XTTS_LANGUAGE = os.getenv(XTTS_LANGUAGE_ENV, XTTS_DEFAULT_LANGUAGE)
    config.XTTS_TIMEOUT_S = float(os.getenv(XTTS_TIMEOUT_S_ENV, str(XTTS_DEFAULT_TIMEOUT_S)))
