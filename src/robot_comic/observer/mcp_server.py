...
Run it where the agent runs (your coding laptop), pointed at the JSONL the app
writes (``ROBOT_INSTRUMENTATION=trace`` + ``ROBOT_EVENT_LOG`` on the app side)::
...
See ``docs/closing-the-loop.md`` for the full design and the planned Tier 1/2
tools (audio witness, visual witness, ``robot_play_prompt`` actuator).
"""
