...
from reachy_mini_conversation_app.profiles.linus.github_add import GitHubAddTool
...
class TestGitHubAddToolExecution:
    ...
    async def test_github_add_repo_not_found(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_add.REPOS_DIR", repos_dir):
    ...
    async def test_github_add_not_git_repo(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_add.REPOS_DIR", repos_dir):
            with patch("reachy_mini_conversation_app.profiles.linus.github_add.Repo") as mock_repo:
    ...
    async def test_github_add_files_success(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_add.REPOS_DIR", repos_dir):
            with patch("reachy_mini_conversation_app.profiles.linus.github_add.Repo", return_value=mock_repo):
