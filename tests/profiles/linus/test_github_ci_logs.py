...
from reachy_mini_conversation_app.profiles.linus.github_ci_logs import GitHubCILogsTool
...
class TestGitHubCILogsGetFullRepoName:
    ...
    def test_get_full_repo_name_with_default_owner(self) -> None:
        ...
        tool = GitHubCILogsTool()
        with patch("reachy_mini_conversation_app.profiles.linus.github_ci_logs.config") as mock_config:
...
class TestGitHubCILogsToolExecution:
    ...
    async def test_github_ci_logs_run_not_found(self, mock_deps: ToolDependencies) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_ci_logs.config") as mock_config:
            mock_config.GITHUB_TOKEN = "test-token"
            with patch("reachy_mini_conversation_app.profiles.linus.github_ci_logs.Github", return_value=mock_github):
    ...
    async def test_github_ci_logs_success(self, mock_deps: ToolDependencies) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_ci_logs.config") as mock_config:
            mock_config.GITHUB_TOKEN = "test-token"
            with patch("reachy_mini_conversation_app.profiles.linus.github_ci_logs.Github", return_value=mock_github):
                with patch("reachy_mini_conversation_app.profiles.linus.github_ci_logs.requests.get", return_value=mock_response):
                    result = await tool(mock_deps, repo="owner/repo", run_id=123)
    ...
    async def test_github_ci_logs_request_exception(self, mock_deps: ToolDependencies) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_ci_logs.config") as mock_config:
            mock_config.GITHUB_TOKEN = "test-token"
            with patch("reachy_mini_conversation_app.profiles.linus.github_ci_logs.Github", return_value=mock_github):
                with patch("reachy_mini_conversation_app.profiles.linus.github_ci_logs.requests.get") as mock_get:
