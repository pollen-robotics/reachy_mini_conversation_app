...
from reachy_mini_conversation_app.profiles.linus.github_clone import GitHubCloneTool
...
class TestGitHubCloneToolHelpers:
    ...
    def test_get_full_repo_name_with_default_owner(self) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_clone.config") as mock_config:
...
class TestGitHubCloneToolExecution:
    ...
    async def test_github_clone_already_exists(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_clone.REPOS_DIR", repos_dir):
            with patch("reachy_mini_conversation_app.profiles.linus.github_clone.config") as mock_config:
    ...
    async def test_github_clone_success(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_clone.REPOS_DIR", repos_dir):
            with patch("reachy_mini_conversation_app.profiles.linus.github_clone.config") as mock_config:
                ...
                with patch("reachy_mini_conversation_app.profiles.linus.github_clone.Repo") as mock_repo_class:
