...
from reachy_mini_conversation_app.profiles.linus.github_close_issue import GitHubCloseIssueTool
...
class TestGitHubCloseIssueToolExecution:
    ...
    async def test_github_close_issue_no_token(self, mock_deps: ToolDependencies) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_close_issue.config") as mock_config:
    ...
    async def test_github_close_issue_close_completed(self, mock_deps: ToolDependencies) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_close_issue.config") as mock_config:
            mock_config.GITHUB_TOKEN = "test-token"
            with patch("reachy_mini_conversation_app.profiles.linus.github_close_issue.Github", return_value=mock_github):
                result = await tool(
                    mock_deps,
                    repo="owner/repo",
                    issue_number=1,
