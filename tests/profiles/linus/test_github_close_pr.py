...
from reachy_mini_conversation_app.profiles.linus.github_close_pr import GitHubClosePRTool
...
class TestGitHubClosePRGetFullRepoName:
    ...
    def test_get_full_repo_name_with_default_owner(self) -> None:
        ...
        tool = GitHubClosePRTool()
        with patch("reachy_mini_conversation_app.profiles.linus.github_close_pr.config") as mock_config:
...
class TestGitHubClosePRToolExecution:
    ...
    async def test_github_close_pr_close_success(self, mock_deps: ToolDependencies) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_close_pr.config") as mock_config:
            mock_config.GITHUB_TOKEN = "test-token"
            with patch("reachy_mini_conversation_app.profiles.linus.github_close_pr.Github", return_value=mock_github):
                result = await tool(
                    mock_deps,
                    repo="owner/repo",
                    pr_number=1,
