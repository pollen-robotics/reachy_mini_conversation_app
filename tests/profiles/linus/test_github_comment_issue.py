...
from reachy_mini_conversation_app.profiles.linus.github_comment_issue import GitHubCommentIssueTool
...
class TestGitHubCommentIssueGetFullRepoName:
    ...
    def test_get_full_repo_name_with_default_owner(self) -> None:
        ...
        tool = GitHubCommentIssueTool()
        with patch("reachy_mini_conversation_app.profiles.linus.github_comment_issue.config") as mock_config:
...
class TestGitHubCommentIssueToolExecution:
    ...
    async def test_github_comment_issue_success(self, mock_deps: ToolDependencies) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_comment_issue.config") as mock_config:
            mock_config.GITHUB_TOKEN = "test-token"
            with patch("reachy_mini_conversation_app.profiles.linus.github_comment_issue.Github", return_value=mock_github):
                result = await tool(
                    mock_deps,
                    repo="owner/repo",
                    issue_number=1,
