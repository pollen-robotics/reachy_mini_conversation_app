...
from reachy_mini_conversation_app.profiles.linus.github_commit import COMMIT_TYPES, GitHubCommitTool
...
class TestGitHubCommitToolExecution:
    ...
    async def test_github_commit_repo_not_found(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_commit.REPOS_DIR", repos_dir):
    ...
    async def test_github_commit_not_git_repo(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_commit.REPOS_DIR", repos_dir):
            with patch("reachy_mini_conversation_app.profiles.linus.github_commit.Repo") as mock_repo:
    ...
    async def test_github_commit_invalid_type(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_commit.REPOS_DIR", repos_dir):
            with patch("reachy_mini_conversation_app.profiles.linus.github_commit.Repo", return_value=mock_repo):
                with patch("reachy_mini_conversation_app.profiles.linus.github_commit.config") as mock_config:
    ...
    async def test_github_commit_success(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_commit.REPOS_DIR", repos_dir):
            with patch("reachy_mini_conversation_app.profiles.linus.github_commit.Repo", return_value=mock_repo):
                with patch("reachy_mini_conversation_app.profiles.linus.github_commit.config") as mock_config:
                    mock_config.GITHUB_DEFAULT_OWNER = None
                    with patch("reachy_mini_conversation_app.profiles.linus.github_commit.load_commit_rules", return_value=None):
...
class TestGenerateWithClaude:
    ...
    def test_generate_with_claude_success(self) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_commit.config") as mock_config:
            ...
            with patch("reachy_mini_conversation_app.profiles.linus.github_commit.anthropic.Anthropic") as mock_client_class:
...
class TestGenerateWithOpenAI:
    ...
    def test_generate_with_openai_success(self) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_commit.config") as mock_config:
            ...
            with patch("reachy_mini_conversation_app.profiles.linus.github_commit.openai.OpenAI") as mock_client_class:
                mock_client = MagicMock()
    ...
    async def test_github_commit_checks_failed(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_commit.REPOS_DIR", repos_dir):
            with patch("reachy_mini_conversation_app.profiles.linus.github_commit.Repo", return_value=mock_repo):
                with patch("reachy_mini_conversation_app.profiles.linus.github_commit.config") as mock_config:
                    mock_config.GITHUB_DEFAULT_OWNER = None
                    with patch("reachy_mini_conversation_app.profiles.linus.github_commit.load_commit_rules", return_value=mock_rules):
                        with patch("reachy_mini_conversation_app.profiles.linus.github_commit.run_pre_commit_checks", return_value=mock_check_results):
                            with patch("reachy_mini_conversation_app.profiles.linus.github_commit.format_check_results", return_value="Check failed"):
                                result = await tool(
                                    mock_deps,
                                    repo="myrepo",
                                    type="feat",
