...
from reachy_mini_conversation_app.profiles.linus.github_discard import GitHubDiscardTool
...
class TestGitHubDiscardToolExecution:
    ...
    async def test_github_discard_repo_not_found(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_discard.REPOS_DIR", repos_dir):
    ...
    async def test_github_discard_not_git_repo(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_discard.REPOS_DIR", repos_dir):
            with patch("reachy_mini_conversation_app.profiles.linus.github_discard.Repo") as mock_repo:
    ...
    async def test_github_discard_all_success(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_discard.REPOS_DIR", repos_dir):
            with patch("reachy_mini_conversation_app.profiles.linus.github_discard.Repo", return_value=mock_repo):
