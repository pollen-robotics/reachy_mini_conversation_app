...
from reachy_mini_conversation_app.profiles.linus.github_edit_file import GitHubEditFileTool
...
class TestGitHubEditFileToolExecution:
    ...
    async def test_github_edit_file_repo_not_found(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_edit_file.REPOS_DIR", repos_dir):
    ...
    async def test_github_edit_file_preview_with_claude(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_edit_file.REPOS_DIR", repos_dir):
            with patch("reachy_mini_conversation_app.profiles.linus.github_edit_file.config") as mock_config:
                mock_config.ANTHROPIC_API_KEY = "test-key"
                mock_config.ANTHROPIC_MODEL = "claude-sonnet-4-20250514"
                with patch("reachy_mini_conversation_app.profiles.linus.github_edit_file.anthropic.Anthropic", return_value=mock_client):
                    result = await tool(
                        mock_deps,
                        repo="myrepo",
                        path="target.py",
    ...
    async def test_github_edit_file_with_openai(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_edit_file.REPOS_DIR", repos_dir):
            with patch("reachy_mini_conversation_app.profiles.linus.github_edit_file.config") as mock_config:
                ...
                with patch("reachy_mini_conversation_app.profiles.linus.github_edit_file.openai.OpenAI", return_value=mock_client):
                    result = await tool(
                        mock_deps,
                        repo="myrepo",
                        path="target.py",
...
class TestGitHubEditFileAPIErrors:
    ...
    async def test_claude_api_exception(self, mock_deps: ToolDependencies, setup_repo: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_edit_file.REPOS_DIR", setup_repo):
            with patch("reachy_mini_conversation_app.profiles.linus.github_edit_file.config") as mock_config:
                mock_config.ANTHROPIC_API_KEY = "test-key"
                mock_config.ANTHROPIC_MODEL = "claude-sonnet-4-20250514"
                with patch("reachy_mini_conversation_app.profiles.linus.github_edit_file.anthropic.Anthropic") as mock_cls:
                    mock_client = MagicMock()
    ...
    async def test_openai_api_exception(self, mock_deps: ToolDependencies, setup_repo: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_edit_file.REPOS_DIR", setup_repo):
            with patch("reachy_mini_conversation_app.profiles.linus.github_edit_file.config") as mock_config:
                mock_config.OPENAI_API_KEY = "test-key"
                with patch("reachy_mini_conversation_app.profiles.linus.github_edit_file.openai.OpenAI") as mock_cls:
                    mock_client = MagicMock()
