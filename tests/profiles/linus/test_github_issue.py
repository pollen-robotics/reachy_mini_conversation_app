...
from reachy_mini_conversation_app.profiles.linus.github_issue import GitHubIssueTool
...
class TestGetFullRepoName:
    ...
    def test_full_repo_name_with_default_owner(self) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_issue.config") as mock_config:
...
class TestGitHubIssueToolExecution:
    ...
    async def test_github_issue_success(self, mock_deps: ToolDependencies) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_issue.config") as mock_config:
            mock_config.GITHUB_TOKEN = "test-token"
            with patch("reachy_mini_conversation_app.profiles.linus.github_issue.Github", return_value=mock_github):
                result = await tool(
                    mock_deps,
                    repo="owner/repo",
                    title="Test Issue",
