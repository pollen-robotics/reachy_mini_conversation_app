...
from reachy_mini_conversation_app.profiles.linus.github_list_issues import GitHubListIssuesTool
...
class TestGitHubListIssuesToolExecution:
    ...
    async def test_github_list_issues_no_token(self, mock_deps: ToolDependencies) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_list_issues.config") as mock_config:
    ...
    async def test_github_list_issues_success(self, mock_deps: ToolDependencies) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_list_issues.config") as mock_config:
            mock_config.GITHUB_TOKEN = "test-token"
            with patch("reachy_mini_conversation_app.profiles.linus.github_list_issues.Github", return_value=mock_github):
                result = await tool(mock_deps, repo="owner/repo")
