...
from reachy_mini_conversation_app.profiles.linus.github_list_repos import GitHubListReposTool
...
class TestGitHubListReposToolExecution:
    ...
    async def test_github_list_repos_dir_not_exists(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_list_repos.REPOS_DIR", nonexistent):
    ...
    async def test_github_list_repos_empty(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_list_repos.REPOS_DIR", repos_dir):
            result = await tool(mock_deps)
