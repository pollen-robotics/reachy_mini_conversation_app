...
from reachy_mini_conversation_app.profiles.linus.github_log import GitHubLogTool
...
class TestGitHubLogToolExecution:
    ...
    async def test_github_log_repo_not_found(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_log.REPOS_DIR", repos_dir):
    ...
    async def test_github_log_not_git_repo(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_log.REPOS_DIR", repos_dir):
            with patch("reachy_mini_conversation_app.profiles.linus.github_log.Repo") as mock_repo:
    ...
    async def test_github_log_default(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_log.REPOS_DIR", repos_dir):
            with patch("reachy_mini_conversation_app.profiles.linus.github_log.Repo", return_value=mock_repo):
