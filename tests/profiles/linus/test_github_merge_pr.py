...
from reachy_mini_conversation_app.profiles.linus.github_merge_pr import GitHubMergePRTool
...
class TestGitHubMergePRGetFullRepoName:
    ...
    def test_get_full_repo_name_with_default_owner(self) -> None:
        ...
        tool = GitHubMergePRTool()
        with patch("reachy_mini_conversation_app.profiles.linus.github_merge_pr.config") as mock_config:
...
class TestGitHubMergePRToolExecution:
    ...
    async def test_github_merge_pr_success(self, mock_deps: ToolDependencies) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_merge_pr.config") as mock_config:
            mock_config.GITHUB_TOKEN = "test-token"
            with patch("reachy_mini_conversation_app.profiles.linus.github_merge_pr.Github", return_value=mock_github):
                result = await tool(
                    mock_deps,
                    repo="owner/repo",
                    pr_number=1,
