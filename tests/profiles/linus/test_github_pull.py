...
from reachy_mini_conversation_app.profiles.linus.github_pull import GitHubPullTool
...
class TestGitHubPullToolExecution:
    ...
    async def test_github_pull_repo_not_found(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_pull.REPOS_DIR", repos_dir):
    ...
    async def test_github_pull_not_git_repo(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_pull.REPOS_DIR", repos_dir):
            with patch("reachy_mini_conversation_app.profiles.linus.github_pull.Repo") as mock_repo:
    ...
    async def test_github_pull_success(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_pull.REPOS_DIR", repos_dir):
            with patch("reachy_mini_conversation_app.profiles.linus.github_pull.Repo", return_value=mock_repo):
