...
from reachy_mini_conversation_app.profiles.linus.github_reset import GitHubResetTool
...
class TestGitHubResetToolExecution:
    ...
    async def test_github_reset_repo_not_found(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_reset.REPOS_DIR", repos_dir):
    ...
    async def test_github_reset_not_git_repo(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_reset.REPOS_DIR", repos_dir):
            with patch("reachy_mini_conversation_app.profiles.linus.github_reset.Repo") as mock_repo:
    ...
    async def test_github_reset_soft_success(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_reset.REPOS_DIR", repos_dir):
            with patch("reachy_mini_conversation_app.profiles.linus.github_reset.Repo", return_value=mock_repo):
