...
from reachy_mini_conversation_app.profiles.linus.github_rm import GitHubRmTool
...
class TestGitHubRmToolExecution:
    ...
    async def test_github_rm_repo_not_found(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_rm.REPOS_DIR", repos_dir):
    ...
    async def test_github_rm_not_git_repo(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_rm.REPOS_DIR", repos_dir):
            with patch("reachy_mini_conversation_app.profiles.linus.github_rm.Repo") as mock_repo:
    ...
    async def test_github_rm_file_success(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_rm.REPOS_DIR", repos_dir):
            with patch("reachy_mini_conversation_app.profiles.linus.github_rm.Repo", return_value=mock_repo):
    ...
    async def test_github_rm_dir_local_modifications_error(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.profiles.linus.github_rm.REPOS_DIR", repos_dir):
            with patch("reachy_mini_conversation_app.profiles.linus.github_rm.Repo", return_value=mock_repo):
                with patch("reachy_mini_conversation_app.profiles.linus.github_rm.shutil.rmtree") as mock_rmtree:
