...
@pytest.mark.asyncio
async def test_gemini_restart_cancels_previous_handler_owned_reconnect(monkeypatch: pytest.MonkeyPatch) -> None:
    """A second Gemini restart should not orphan the first handler-owned reconnect task."""
    handler = GeminiLiveHandler(ToolDependencies(reachy_mini=MagicMock(), movement_manager=MagicMock()))
    handler.client = MagicMock()
    cancelled_tasks: list[asyncio.Task[Any] | None] = []

    async def pending_start_up() -> None:
    ...
    monkeypatch.setattr(handler, "start_up", pending_start_up)
...
@pytest.mark.asyncio
async def test_gemini_bound_tool_result_drops_stale_session() -> None:
    """A tool result from an old lifecycle should not be sent to the replacement Gemini session."""
    handler = GeminiLiveHandler(ToolDependencies(reachy_mini=MagicMock(), movement_manager=MagicMock()))
    stale_session = _FakeSession([], handler._stop_event)
    replacement_session = _FakeSession([], handler._stop_event)
    handler.session = replacement_session
    ...
    assert replacement_session.tool_responses == []
    assert stale_session.tool_responses == []
