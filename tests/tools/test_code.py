...
class TestCodeToolExecution:
    ...
    async def test_code_missing_api_key(self, mock_deps: ToolDependencies) -> None:
        ...
        with patch("reachy_mini_conversation_app.tools.code.config") as mock_config:
    ...
    async def test_code_repo_not_found(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.tools.code.config") as mock_config:
            mock_config.ANTHROPIC_API_KEY = "test-key"
            with patch("reachy_mini_conversation_app.tools.code.REPOS_DIR", tmp_path):
    ...
    async def test_code_success_to_reachy_code(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.tools.code.config") as mock_config:
            ...
            with patch("reachy_mini_conversation_app.tools.code.CODE_OUTPUT_DIR", tmp_path):
                with patch("reachy_mini_conversation_app.tools.code.anthropic.Anthropic", return_value=mock_client):
