...
from reachy_mini_conversation_app.tools.github_pr_comment import GitHubPRCommentTool
...
class TestGitHubPRCommentGetFullRepoName:
    ...
    def test_get_full_repo_name_with_default_owner(self) -> None:
        ...
        tool = GitHubPRCommentTool()
        with patch("reachy_mini_conversation_app.tools.github_pr_comment.config") as mock_config:
...
class TestGitHubPRCommentToolExecution:
    ...
    async def test_github_pr_comment_success(self, mock_deps: ToolDependencies) -> None:
        ...
        with patch("reachy_mini_conversation_app.tools.github_pr_comment.config") as mock_config:
            mock_config.GITHUB_TOKEN = "test-token"
            with patch("reachy_mini_conversation_app.tools.github_pr_comment.Github", return_value=mock_github):
                result = await tool(
                    mock_deps,
                    repo="owner/repo",
                    pr_number=1,
