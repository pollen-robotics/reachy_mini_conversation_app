    ...
    async def test_github_read_file_analyze_with_claude(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.tools.github_read_file.REPOS_DIR", repos_dir):
            with patch("reachy_mini_conversation_app.tools.github_read_file.config") as mock_config:
                ...
                with patch("reachy_mini_conversation_app.tools.github_read_file.anthropic.Anthropic", return_value=mock_client):
    ...
    async def test_github_read_file_analyze_with_openai(self, mock_deps: ToolDependencies, tmp_path: Path) -> None:
        ...
        with patch("reachy_mini_conversation_app.tools.github_read_file.REPOS_DIR", repos_dir):
            with patch("reachy_mini_conversation_app.tools.github_read_file.config") as mock_config:
                ...
                with patch("reachy_mini_conversation_app.tools.github_read_file.openai.OpenAI", return_value=mock_client):
